...
- **Persistence.** `config set` of a dynamic entry key refuses with [`UnaddressableKey`](https://docs.rs/clapfig/latest/clapfig/enum.ClapfigError.html) — the same class of error a named MapOf already used. This epic does not invent addressing for dynamic keys.
