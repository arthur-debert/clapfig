...
- **Strict mode** — unknown keys error with file path, key name, and (for TOML sources) line number (on by default), with a cascading per-subtree override system and a per-key callback for the edge cases
