...
pub enum LeafTypeStatic {
    ...
    /// attribute tightens it. `isize` carries target-side pointer-width
    /// bounds, so a 32-bit target rejects values the `i64` value model can
    /// hold but `isize` cannot while a 64-bit target remains unbounded.
    /// `u64` carries `min: Some(0)` with an open upper end —
    ...
    /// and there is no faithful intermediate. `usize` is target-side too:
    /// 64-bit targets keep an open upper end, while narrower targets emit
    /// `Some(usize::MAX as i64)`. Field types like `u64` are accepted
    /// because they are convenient and round-trip correctly for the
    /// overwhelming majority of values; callers who need the full
    /// unsigned-64 range should store them as `String` and parse
    /// explicitly.
    ///
